...
演进。如图7-1所示，为本地部署时CPU与 GPU的性能对比。

图 7-1 CPU vs GPU 本地 LLM 推理性能对比
...
存储
(SSD)
1TB NVMe SSD 2TB+ NVMe SSD 多块NVMe SSD组 RAID 高速NVMe SSD能极大缩
短模型和数据的加载时
间。
...
的AI 应用封装成一个标准化的网络服务，通过API（应用程序接口）对外提供能力，并借
...
图 7-7 标准模型与延迟优化模型性能对比（以 Claude 3.5 Haiku 为例的示意数据）
